...
fn main() -> Result<()> {
    ...
    let pp: PublicParams<E1, E2, FibStep> =
        decode_b64(&vkf.vk_b64).map_err(|e| anyhow::anyhow!("decode vk_b64: {e}"))?;
    // RecursiveSNARK<E1,E2,FibStep>
    let rs: RecursiveSNARK<E1, E2, FibStep> =
        decode_b64(&pf.proof_b64).map_err(|e| anyhow::anyhow!("decode proof_b64: {e}"))?;

    // 4) Verifikasi
    let z0 = vec![F1::ONE, F1::ONE]; // harus sama dengan generator
    let z_final = rs
        .verify(&pp, pf.num_steps as usize, &z0)
